...
public class LessonService {
    ...
    public List<Key<Block>> update_blocks(long task_id, List<SaveBlockModel> blocks) {
        ...
        for(int i = 0; i < blocks.size(); i++) {
            ...
            if(new_block.getBlock_id() == null){
                new_block.setBlock_id(Block.NULL_BLOCK_ID);
            }
            ...
            if(new_block.isCan_edit()){
                ...
                if(new_block.getChildren().size() == old_block.getChildren().size()){
                    ...
                    for(int j = 0; j < new_block.getChildren().size(); j++){
                        Long old_value = old_block.getChildren().get(j).getBlock_id();
                        Long new_value = new_block.getChildren().get(j).getBlock_id();
                        if(new_value == null){
                            if(old_value.equals(Block.NULL_BLOCK_ID)){
                                continue;
                            }else {
                ...
                if(!same && old_b.isOriginal()){
                    ...
                    Block b = new Block();
                    b.setOriginal(false);
                    b.setChildren(update_blocks(task_id, new_block.getChildren()));
                    Key block_key = blockService.save_block_model_work(b, new_block);
                    // Add the key to the list
                    block_keys.add(block_key);
                }else if(!same && !old_b.isOriginal()){
                    //not original block but also not the same
                    if(old_b.getBlock_id().equals(Block.NULL_BLOCK_ID)){
                        //make new block
                        new_block.setBlock_id(null);
                    }
                    ...
                    old_b.setChildren(update_blocks(task_id, new_block.getChildren()));
                    Key block_key = blockService.save_block_model_work(old_b, new_block);
                    block_keys.add(block_key);
                }
            }
            else{
